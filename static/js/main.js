...
$(function () {
  ...
  $win.on('scroll', function () {
    if ($win.width() >= 992) {
      ...
      const $secondaryNavbar = $('.secondary-nav')
  ...
  $('.increase-qty').on('click', function () {
    const $qty = $(this).parent().find('.qty-input')
  ...
  if ($('.animated-Progressbars').length > 0) {
    $(window).on('scroll', function () {
      const skillsOffset = $('.animated-Progressbars').offset().top - 160
      const skillsHight = $(this).outerHeight()
  ...
  const $rangeSliderResult = $('#rangeSliderResult')
  $rangeSlider.slider({
    range: true,
    min: 0,
    max: 300,
